...
def is_admin(view_func):
    ...
    def _wrapped_view_func(request, *args, **kwargs):
        if not request.user.is_superuser:
            return HttpResponse(
                u'<h3>Necesitas privilegios de aministrador para realizar esta acción</h3>')
        return view_func(request, *args, **kwargs)

    return _wrapped_view_func


def permission_resource_required(function=None, template_name='access/dashboard.html',
